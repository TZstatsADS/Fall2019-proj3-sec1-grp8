...
nmf_total.create_nmf(test_size=400)
...
y_train_emot1 -= 7*np.ones(len(y_train_emot1), dtype='int64')

x_test_nmf = nmf_features [2]
y_test_cat = nmf_features[3][:,0]
y_test_emot1 = nmf_features[3][:,1]
y_test_emot1 -= 7*np.ones(len(y_test_emot1), dtype='int64')
...
history = model.fit(x_train_nmf, y_train_emot1, validation_data = (x_test_nmf,y_test_emot1), batch_size = 24,epochs = 50)
